...
static char *mkstring(string_t *string, char *name)
{
	...
	string->string = dnrd_strdup(name);
...
static int create_nameip(nameip_t *nameip, char *ip)
{
	...
	if (inet_aton(ip, &ipnum) == 0) {
		log_msg("invalid ip number: %s", ip);
		return (0);
	}
	nameip->ipnum = ntohl(ipnum.s_addr);
	// Swap the bytes.  Don't assume 32-bit integers.
	ipnum.s_addr = (((nameip->ipnum & 0xff000000) >> 24) | ((nameip->ipnum & 0x00ff0000) >> 8) | ((nameip->ipnum & 0x0000ff00) << 8) | ((
...
static dnsrec_t *alloc_dnsrec(char *name)
{
	...
	if (mkstring(&rec->object, name) == NULL) {
		return NULL;
	} else {
		return (rec);
	}
...
char *get_hostname(char **from, char *domain, char *name, unsigned int size)
{
	int	 len;
	char word[100];
...
int read_configuration(char *filename)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		if (isdigit((int)(*p))) {
			...
			if (*get_word(&p, filename, sizeof(filename)) == 0) {
				continue;
			}
...
static char *compile_int(dnsheader_t *x, int number)
{
...
static char *compile_long(dnsheader_t *x, long number)
{
...
static char *compile_objectname(dnsheader_t *x)
{
	*x->here++ = 0xC0;
	*x->here++ = 0x0C;
	...
	if (auto_authority != 0) {
		...
		for (i = 0; i < dbc; i++) {
			if (dbv[i]->type == DNS_NAMEIP) {
				...
				if ((domain = strchr(arpaname, '.')) == NULL) {
					continue;
				}
