				 ...
				 unsigned int *srvidx)
{
	...
	/* Default case.  Send to a server until it "times out". */
	{
		...
		*srvidx = serv_act;
		...
		log_debug("Default case Forwarding the query to DNS server %s",
				  inet_ntoa(dns_srv[*srvidx].addr.sin_addr));
	}
	return 1;
}
...
void run(void)
{
	...
	while (1) {
		...
		/* Handle errors */
		if (retn < 0) {
			printf("\n\r %s %d select returned %d", __FUNCTION__, __LINE__, retn);
			continue;
		} else if (retn == 0) {
		...
		for (i = serv_act, j = 0; (unsigned int)j < serv_cnt; j++) {
